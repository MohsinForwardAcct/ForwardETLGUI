...
class FormEditScreen(Container):
    ...
    def __init__(self, page, download, Query, ScreenName, ReportTitle):
        super().__init__()
        ...
        BrdxVariablesList = ['UID','Status']
        if self.ScreenName == 'BrdxTemplates': self.TableData = self.TableData.drop(BrdxVariablesList, axis=1)
        elif self.ScreenName == 'AddNewTemplate': self.BuildTableData(); self.TableData = self.TableData.drop(BrdxVariablesList, axis=1)
    ...
    def BuildTableData(self):
        DataValue = self.ReportTitle.split('-'); self.CONID = DataValue[0].strip(); self.PremiumCategory = DataValue[1].strip()
        self.TableData.loc['CONID'] = self.CONID
        self.TableData.loc[self.TableData.index, 'PremiumCategory'] = self.PremiumCategory
        self.TableData.loc[self.TableData.index, 'ColumnSequence'] = 1
        self.TableData.loc[self.TableData.index, 'ColumnOutput'] = None
        self.TableData.loc[self.TableData.index, 'DataType'] = None
        self.TableData.loc[self.TableData.index, 'AllowNull'] = None
    ...
    def GetFormTable(self):
        ...
        for index, rows in self.TableData.iterrows():
            self.RowsData.append(DataRow(cells=[DataCell(DragTarget(on_accept=self.GetDropDrag,content=Draggable(content=Container(
    ...
    async def GetDropDrag(self, e:DragTargetAcceptEvent):
        src = self.FormScreen.page.get_control(e.src_id)
        SourceOutput = src.content.content.value
        DestinationOutput = e.control.content.content.content.value
        print(SourceOutput, DestinationOutput)
        if SourceOutput in self.TableData['ColumnOutput'].values:
            SourceSequence = self.TableData[self.TableData['ColumnOutput'] == SourceOutput]['ColumnSequence'].values[0]
            DestinationSequence = self.TableData[self.TableData['ColumnOutput'] == DestinationOutput]['ColumnSequence'].values[0]
            self.TableData.loc[self.TableData['ColumnSequence'] == SourceSequence, 'ColumnOutput'] = DestinationOutput
            self.TableData.loc[self.TableData['ColumnSequence'] == DestinationSequence, 'ColumnOutput'] = SourceOutput
    ...
    async def PageLogicTables(self,e):
        ...
        # print(e.control.cells[valuecounter].content.content.content.content.value)
        self.ColumnSequence = e.control.cells[2].content.content.content.content.value
        self.ColumnOutput = e.control.cells[3].content.content.content.content.value
        for Value in self.FormFields.content.controls[0].controls:
            ...
            self.FieldData = e.control.cells[counter].content.content.content.content.value
        ...
        for cols in TableFrameColumns:
            ...
            if self.ColumnName == 'AllowNull' or self.ColumnName == 'DataType':
                ColumnNames.append(Container(expand=2,height=45,bgcolor="#ebebeb",border_radius=6,padding=8,content=Column(spacing=1,controls=[
                    ...
                    options=self.GetDropDownOptions()), TextField(border_color="transparent",color="black",height=15,text_size=13,disabled='True')])))
            else:    
                ColumnNames.append(Container(expand=2,height=45,bgcolor="#ebebeb",border_radius=6,padding=8,content=Column(spacing=1,controls=[
    ...
    async def DropdownValue(self,e):
        ...
        if e.control.label == 'AllowNull': 
            if e.control.value == 'null': self.AllowNull = None
            else: self.AllowNull = e.control.value
            self.FormFields.content.controls[0].controls[5].content.controls[1].value = self.AllowNull 
        elif e.control.label == 'DataType': 
            self.DataType = e.control.value
            self.FormFields.content.controls[0].controls[4].content.controls[1].value = self.DataType
        await self.FormFields.update_async()
        
    def GetDropDownOptions(self):
        if self.ColumnName == 'AllowNull': self.OptionsData = ['not null','null']
        elif self.ColumnName == 'DataType': self.OptionsData = ['float','datetime','nvarchar(100)','nvarchar(max)']
        options = []
        for self.FieldValue in self.OptionsData:
            options.append(dropdown.Option(self.FieldValue))
        return options

    async def UpdateValue(self,e):
        self.ColumnSequence = self.FormFields.content.controls[0].controls[2].content.controls[1].value 
        self.ColumnOutput = self.FormFields.content.controls[0].controls[3].content.controls[1].value 
        self.DataType = self.FormFields.content.controls[0].controls[4].content.controls[1].value
        self.AllowNull = self.FormFields.content.controls[0].controls[5].content.controls[1].value 
        print(self.ColumnOutput, self.ColumnSequence, self.DataType, self.AllowNull)
        self.TableData.loc[self.TableData['ColumnSequence'] == self.ColumnSequence, 'ColumnOutput'] = self.ColumnOutput
        self.TableData.loc[self.TableData['ColumnSequence'] == self.ColumnSequence, 'AllowNull'] = self.AllowNull
        self.TableData.loc[self.TableData['ColumnSequence'] == self.ColumnSequence, 'DataType'] = self.DataType
        await self.GetFormScreenUpdate()    
    ...
    def GetFormButton(self):
        FormButton = Container(alignment=alignment.center,content= Row(controls=[
            ...
            ElevatedButton(on_click=self.GetAddLine,bgcolor="#AD1457",color="white",content=Row(controls=[Icon(name=icons.ADD_ROUNDED,size=12, color=colors.WHITE),
                    Text("Add Line", size=10, weight="bold")]),style=ButtonStyle(shape={"":RoundedRectangleBorder(radius=6)},color={"":"white"}),height=30,width=100),
            ...
            ElevatedButton(on_click=self.GetDeleteLine,bgcolor="#AD1457",color="white",content=Row(controls=[Icon(name=icons.DELETE_ROUNDED,size=12, color=colors.WHITE),
    ...
    async def GetActivated(self, e):
        self.ActionCall = 'ActivateTemplate'
    ...
    async def GetValidate(self, e):
        ...
        Test1 = self.TableData['ColumnOutput'].values
        Test2 = self.TableData['DataType'].values
        ...
        if Validation1 == 'Pass' and Validation2 == 'Pass':
            self.FormFields.content.controls[2].controls[0].content.controls[0].disabled = 'False'
            await self.FormFields.update_async()     
        else: 
            self.GetAlertMessage2('Valiation Failed', f'ColumOutput and DataType fields cannot be blank!')
    ...
    async def GetEditModeOff(self,e):
        self.ScreenName = 'BrdxTemplates'
        self.ReportScreen = GF.FormScreen(self.page, self.Download, self.Query, self.ScreenName, self.ReportTitle).GetFormScreen()
        self.FormScreen.controls.clear()
        self.FormScreen.controls.append(self.ReportScreen)
        await self.FormScreen.update_async()

    async def GetAddLine(self,e):
        self.SequenceCount = len(self.TableData['ColumnSequence']) + 1
        self.TemplateData.loc[self.TemplateData.index, 'ColumnSequence'] = self.SequenceCount
        self.TemplateData.loc[self.TemplateData.index, 'ColumnOutput'] = None
        self.TableData = pd.concat([self.TableData, self.TemplateData]) 
        await self.GetFormScreenUpdate()

    async def GetDeleteLine(self, e):
        self.ActionCall = 'DeleteLine'
